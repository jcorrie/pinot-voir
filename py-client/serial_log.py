...
def is_audio_data(data):
    ...
    try:
        ...
        samples = struct.unpack(f'<{len(data)//2}h', data)  # Changed to 'h' for signed
...
print(f"Duration of recorded audio: {len(unsigned_data) / (FRAME_RATE * SAMPLE_WIDTH * CHANNELS):.2f} seconds")